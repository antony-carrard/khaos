...
func bind(player: Player) -> void:
	if current_player:
		current_player.materials_changed.disconnect(resources_changed.emit)
	...
	if player:
		player.materials_changed.connect(resources_changed.emit)
		...
		resources_changed.emit(player.materials)
